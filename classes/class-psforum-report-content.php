...
class psf_ReportContent {
	...
	private function __construct() {
		...
		// Stopgap in case somehow PSForum becomes inactive after activation
		add_action( 'admin_init', array( 'psf_ReportContent', 'check_for_psforum' ) );
		...
		add_action( 'psf_register_post_statuses',       array( $this, 'register_post_status'         )           );
		...
		add_filter( 'psf_admin_topics_column_headers',  array( $this, 'admin_topics_column_headers'  )           );
		add_action( 'psf_admin_topics_column_data',     array( $this, 'admin_topics_column_data'     ),   10,  2 );

		// Reply column headers
		add_filter( 'psf_admin_replies_column_headers', array( $this, 'admin_replies_column_headers' )           );
		add_action( 'psf_admin_replies_column_data',    array( $this, 'admin_replies_column_data'    ),   10,  2 );

		/************************************************************************
		 * Topics
		 ***********************************************************************/
		// Add status to list of topic statuses
		add_filter( 'psf_get_topic_statuses',           array( $this, 'add_topic_status'      )           );

		// Add admin links
		add_filter( 'psf_topic_admin_links',            array( $this, 'add_topic_admin_links' ),   10,  2 );

		// Report handler
		add_action( 'psf_get_request',                  array( $this, 'toggle_topic_handler'  ),    1     );

		// Add post status to topics query for view=all
		add_filter( 'psf_after_has_topics_parse_args',  array( $this, 'insert_report_status'  )           );

		// Add notice to reported topic
		add_action( 'psf_template_before_single_topic', array( $this, 'output_topic_notice'   )           );
		 ...
		 ***********************************************************************/
		// Add admin links
		add_filter( 'psf_reply_admin_links',            array( $this, 'add_reply_admin_links' ),   10,  2 );

		// Report handler
		add_action( 'psf_get_request',                  array( $this, 'toggle_reply_handler'  ),    1     );

		// Add post status to replies query for view=all
		add_filter( 'psf_after_has_replies_parse_args', array( $this, 'insert_report_status'  )           );

		// Add notice to reported reply
		add_action( 'psf_theme_before_reply_content',   array( $this, 'output_reply_notice'   )           );
	}
	...
	/**
	 * Checks if PSForum is active.
	 *
	 * @since 1.0.5
	 * @return boolean
	 */
	public static function is_psforum_active() {
		return class_exists( 'PSForum' );
	}

	/**
	 * Check for PSForum on activation.
	 *
	 * @since 1.0.5
	 * @return void
	 */
	public static function activation_check() {
	    if ( ! self::is_psforum_active() ) {
	        ...
	        wp_die( __( 'PSForum - Report Content erfordert die Aktivierung von PSForum.', 'psf-report-content' ) );
	    }
	}

	/**
	 * Check for PSForum on admin load.
	 *
	 * @since 1.0.5
	 * @return void
	 */
	public static function check_for_psforum() {
		if ( ! self::is_psforum_active() ) {
			if ( is_plugin_active( plugin_basename( __FILE__ ) ) ) {
				deactivate_plugins( plugin_basename( __FILE__ ) );

				add_action( 'admin_notices', array( 'psf_ReportContent', 'disabled_notice' ) );
	...
	public static function disabled_notice() {
		echo '<div class="updated"><p><strong>' . esc_html__( 'PSForum - Inhalt melden wurde deaktiviert. Dieses Plugin erfordert, dass PS Forum aktiv ist.', 'psf-report-content' ) . '</strong></p></div>';
	}
	...
	public function get_reported_status_id() {
		return apply_filters( 'psf_rc_reported_post_status',    'reported'    );
	}
	...
	public function register_post_status() {
		...
		register_post_status(
			...
			apply_filters( 'psf_rc_register_reported_post_status', array(
				'label'                     => _x( 'Benutzer gemeldet', 'post', 'psforum-report-content' ),
				'label_count'               => _nx_noop( 'Benutzer gemeldet <span class="count">(%s)</span>', 'Benutzer gemeldet <span class="count">(%s)</span>', 'post', 'psforum-report-content' ),
				'public'                    => true,
	...
	public function add_topic_status( $statuses ) {
		...
		$statuses[$this->get_reported_status_id()] = _x( 'Gemeldet', 'Thema als gemeldet markieren', 'psforum-report-content' );
	...
	function get_topic_report_link( $args = '' ) {
		...
		$r = psf_parse_args( $args, array(
			...
			'link_after'   => '',
			'sep'          => ' | ',
			'report_text'    => esc_html__( 'Melden',   'psforum-report-content' ),
			'unreport_text'  => esc_html__( 'Meldung aufheben', 'psforum-report-content' )
		...
		$uri     = add_query_arg( array( 'action' => 'psf_rc_toggle_topic_report', 'topic_id' => $topic->ID ) );
		$uri     = wp_nonce_url( $uri, 'report-topic_' . $topic->ID );
		$classes = array( 'psf-topic-report-link' );
		if ( true === $reported ) {
			$classes[] = 'reported';
		} else {
			$classes[] = 'unreported';
		}
		$retval  = $r['link_before'] . '<a href="' . esc_url( $uri ) . '" class="' . join( ' ', array_map( 'esc_attr', $classes ) ) . '" title="' . __( 'Unangemessene Inhalte melden', 'psforum-report-content' ) . '">' . $display . '</a>' . $r['link_after'];

		return apply_filters( 'psf_rc_get_topic_report_link', $retval, $r );
	}
	 ...
	 * @uses apply_filters() Calls 'psf_is_topic_reported' with the topic id
	 * @return bool True if reported, false if not.
	 */
	public function is_topic_reported( $topic_id = 0 ) {
		$topic_status = psf_get_topic_status( psf_get_topic_id( $topic_id ) ) === $this->get_reported_status_id();
		return (bool) apply_filters( 'psf_rc_is_topic_reported', (bool) $topic_status, $topic_id );
	}
	...
	public function toggle_topic_handler( $action = '' ) {
		...
		switch ( $action ) {
			...
			// Toggle reported
			case 'psf_rc_toggle_topic_report' :
				...
				$failure  = true === $is_reported ? __( '<strong>FEHLER</strong>: Beim Aufheben der Markierung des Themas als gemeldet ist ein Problem aufgetreten.', 'psforum-report-content' ) : __( '<strong>FEHLER</strong>: Beim Melden des Themas ist ein Problem aufgetreten.', 'psforum-report-content' );
		...
		// Handle errors
			...
			psf_add_error( 'psf_rc_toggle_topic', $failure );
		}
	}
	 ...
	 * @uses do_action() Calls 'psf_rc_report_topic' with the topic id
	 * @uses add_post_meta() To add the previous status to a meta
	 * @uses wp_update_post() To update the topic with the new status
	 * @uses do_action() Calls 'psf_rc_reported_topic' with the topic id
	...
	public function report_topic( $topic_id = 0 ) {
		...
		do_action( 'psf_rc_report_topic', $topic_id );
		...
		update_post_meta( $topic_id, '_psf_report_user_id', wp_get_current_user()->ID );

		// Add the original post status as post meta for future restoration
		add_post_meta( $topic_id, '_psf_report_meta_status', $topic->post_status );
		...
		do_action( 'psf_rc_reported_topic', $topic_id );
	 ...
	 * @param int $topic_id Topic id
	 * @uses psf_get_topic() To get the topic
	 * @uses do_action() Calls 'psf_rc_unreport_topic' with the topic id
	 * @uses get_post_meta() To get the previous status
	 * @uses delete_post_meta() To delete the previous status meta
	 * @uses wp_update_post() To update the topic with the new status
	 * @uses do_action() Calls 'psf_rc_unreported_topic' with the topic id
	 * @return mixed False or {@link WP_Error} on failure, topic id on success
	 */
	public function unreport_topic( $topic_id = 0 ) {
		...
		do_action( 'psf_rc_unreport_topic', $topic_id );

		// Get previous status
		$topic_status       = get_post_meta( $topic_id, '_psf_report_meta_status', true );

		// Set previous status
		$topic->post_status = $topic_status;

		// Remove old status meta
		delete_post_meta( $topic_id, '_psf_report_meta_status' );

		// Add the user id of the user who reported
		delete_post_meta( $topic_id, '_psf_report_user_id' );
		...
		do_action( 'psf_rc_unreported_topic', $topic_id );
	...
	public function get_reply_report_link( $args = '' ) {
		...
		$display  = $reported ? $r['unreport_text'] : $r['report_text'];
		$uri      = add_query_arg( array( 'action' => 'psf_rc_toggle_reply_report', 'reply_id' => $reply->ID ) );
		$uri      = wp_nonce_url( $uri, 'report-reply_' . $reply->ID );
		$classes = array( 'psf-reply-report-link' );
		if ( true === $reported ) {
			$classes[] = 'reported';
		} else {
			$classes[] = 'unreported';
		...
		return apply_filters( 'psf_rc_get_reply_report_link', $retval, $r );
	}
	...
	public function is_reply_reported( $reply_id = 0 ) {
		$reply_status = psf_get_reply_status( psf_get_reply_id( $reply_id ) ) === $this->get_reported_status_id();
		return (bool) apply_filters( 'psf_rc_is_reply_reported', (bool) $reply_status, $reply_id );
	}
	...
	public function toggle_reply_handler( $action = '' ) {
		...
		switch ( $action ) {
			...
			// Toggle reported
			case 'psf_rc_toggle_reply_report' :
				...
				$failure  = true === $is_reported ? __( '<strong>FEHLER</strong>: Beim Aufheben der Markierung der Antwort als gemeldet ist ein Problem aufgetreten.', 'psforum-report-content' ) : __( '<strong>FEHLER</strong>: Beim Melden der Antwort ist ein Problem aufgetreten.', 'psforum-report-content' );
				// $view_all = !$is_reported; // Only need this if we want to hide it, like spam
		...
		// Handle errors
			...
			psf_add_error( 'psf_rc_toggle_reply', $failure );
		}
	}
	 ...
	 * @uses do_action() Calls 'psf_rc_reported_reply' with the reply ID
	 * @uses add_post_meta() To add the previous status to a meta
	...
	function report_reply( $reply_id = 0 ) {
		...
		// Execute pre report code
		do_action( 'psf_rc_report_reply', $reply_id );

		// Add the user id of the user who reported
		update_post_meta( $reply_id, '_psf_report_user_id', wp_get_current_user()->ID );

		// Add the original post status as post meta for future restoration
		add_post_meta( $reply_id, '_psf_report_meta_status', $reply->post_status );
		...
		// Execute post report code
		do_action( 'psf_rc_reported_reply', $reply_id );
	 ...
	 * @param int $reply_id Reply id
	 * @uses psf_get_reply() To get the reply
	 * @uses do_action() Calls 'psf_rc_unreport_reply' with the reply ID
	 * @uses get_post_meta() To get the previous status meta
	 * @uses delete_post_meta() To delete the previous status meta
	 * @uses wp_update_post() To insert the updated post
	 * @uses do_action() Calls 'psf_rc_unreported_reply' with the reply ID
	 * @return mixed False or {@link WP_Error} on failure, reply id on success
	 */
	function unreport_reply( $reply_id = 0 ) {
		...
		do_action( 'psf_rc_unreport_reply', $reply_id );

		// Get pre report status
		$reply->post_status = get_post_meta( $reply_id, '_psf_report_meta_status', true );
		...
		delete_post_meta( $reply_id, '_psf_report_meta_status' );

		// Add the user id of the user who reported
		delete_post_meta( $reply_id, '_psf_report_user_id' );
		...
		do_action( 'psf_rc_unreported_reply', $reply_id );
	...
	public function output_topic_notice() {
		...
		echo '<div class="psf-template-notice error psf-rc-topic-is-reported">';
			echo '<p>';
				echo apply_filters( 'psf_rc_topic_notice', __( 'Dieses Thema wurde wegen unangemessenen Inhalts gemeldet', 'psforum-report-content' ) );
			echo '</p>';
		echo '</div>';
	}
	...
	public function output_reply_notice() {
		...
		echo '<div class="error psf-rc-reply-is-reported">';
			echo '<p>';
				echo apply_filters( 'psf_rc_reply_notice', __( '<em>Diese Antwort wurde wegen unangemessenen Inhalts gemeldet.</em>', 'psforum-report-content' ) );
			echo '</p>';
		echo '</div>';
	}
	...
	public function topic_row_actions( $actions, $topic ) {
		...
		if ( current_user_can( 'moderate', $topic->ID ) ) {
			...
			// Report
			$report_uri  = wp_nonce_url( add_query_arg( array( 'topic_id' => $topic->ID, 'action' => 'psf_rc_toggle_topic_report' ), remove_query_arg( array( 'psf_topic_toggle_notice', 'topic_id', 'failed', 'super' ) ) ), 'report-topic_'  . $topic->ID );
			if ( $this->is_topic_reported( $topic->ID ) )
				$actions['report'] = '<a href="' . esc_url( $report_uri ) . '" title="' . esc_attr__( 'Markiere das Thema als nicht gemeldet', 'psforum-report-content' ) . '">' . esc_html__( 'Meldung aufheben', 'psforum-report-content' ) . '</a>';
		}
	...
	public function toggle_topic_admin_handler() {
		...
		if ( psf_is_get_request() && !empty( $_GET['action'] ) && in_array( $_GET['action'], array( 'psf_rc_toggle_topic_report' ) ) && !empty( $_GET['topic_id'] ) ) {
			...
			if ( empty( $topic ) )
				wp_die( __( 'Das Thema wurde nicht gefunden!', 'psforum-report-content' ) );

			if ( !current_user_can( 'moderate', $topic->ID ) ) // What is the user doing here?
				wp_die( __( 'Du hast keine Berechtigung dazu!', 'psforum-report-content' ) );
	...
	public function toggle_topic_notice_admin() {
		...
		// Only proceed if GET is a topic toggle action
		if ( psf_is_get_request() && !empty( $_GET['psf_topic_toggle_notice'] ) && in_array( $_GET['psf_topic_toggle_notice'], array( 'unreported' ) ) && !empty( $_GET['topic_id'] ) ) {
			...
			switch ( $notice ) {
				case 'unreported'    :
					$message = $is_failure === true ? sprintf( __( 'Beim Aufheben der Meldung des Themas "%1$s" ist ein Problem aufgetreten.', 'psforum-report-content' ), $topic_title ) : sprintf( __( 'Thema "%1$s" erfolgreich aufgehoben.', 'psforum-report-content' ), $topic_title );
					break;
			}
	...
	public function reply_row_actions( $actions, $reply ) {
		...
		if ( psf_get_reply_post_type() != get_current_screen()->post_type )
			return $actions;
		...
		if ( current_user_can( 'moderate', $reply->ID ) ) {
			...
			// Report
			$report_uri  = wp_nonce_url( add_query_arg( array( 'reply_id' => $reply->ID, 'action' => 'psf_rc_toggle_reply_report' ), remove_query_arg( array( 'psf_reply_toggle_notice', 'reply_id', 'failed', 'super' ) ) ), 'report-reply_'  . $reply->ID );
			if ( $this->is_reply_reported( $reply->ID ) )
				$actions['report'] = '<a href="' . esc_url( $report_uri ) . '" title="' . esc_attr__( 'Antwort als nicht gemeldet markieren', 'psforum-report-content' ) . '">' . esc_html__( 'Meldung aufheben', 'psforum-report-content' ) . '</a>';
		}
	...
	public function toggle_reply_admin_handler() {
		...
		if ( psf_is_get_request() && !empty( $_GET['action'] ) && in_array( $_GET['action'], array( 'psf_rc_toggle_reply_report' ) ) && !empty( $_GET['reply_id'] ) ) {
			$action    = $_GET['action'];            // What action is taking place?
			$reply_id  = (int) $_GET['reply_id'];    // What's the reply id?
			$success   = false;                      // Flag
			$post_data = array( 'ID' => $reply_id ); // Prelim array
			...
			if ( empty( $reply ) )
				wp_die( __( 'Die Antwort wurde nicht gefunden!', 'psforum-report-content' ) );
	...
	public function toggle_reply_notice_admin() {
		...
		// Only proceed if GET is a reply toggle action
		if ( psf_is_get_request() && !empty( $_GET['psf_reply_toggle_notice'] ) && in_array( $_GET['psf_reply_toggle_notice'], array( 'unreported' ) ) && !empty( $_GET['reply_id'] ) ) {
			...
			switch ( $notice ) {
				case 'unreported'    :
					$message = $is_failure === true ? sprintf( __( 'Beim Aufheben der Meldung der Antwort "%1$s" ist ein Problem aufgetreten.', 'psforum-report-content' ), $reply_title ) : sprintf( __( 'Antwort "%1$s" erfolgreich nicht gemeldet.', 'psforum-report-content' ), $reply_title );
					break;
			}
	...
	public function admin_topics_column_headers( $columns ) {
		...
		if ( !isset($_GET['post_status']) || 'reported' != $_GET['post_status'] || psf_get_topic_post_type() != get_current_screen()->post_type )
			return $columns;

		// Add "reported by" column
		$columns['psf_rc_user'] = __( 'Gemeldet von', 'psforum-report-content' );
	...
	public function admin_topics_column_data( $column, $topic_id ) {
		...
		switch ( $column ) {
			case 'psf_rc_user':
				$user_id = get_post_meta( $topic_id, '_psf_report_user_id', true );
				$user    = $this->get_username( $user_id );
				$link    = '<a class="psf-rc-user-col" href="' . admin_url( 'user-edit.php?user_id=' . intval($user_id) ) . '">' . esc_html( $user ) . '</a>';
				echo $link;
				break;
		}
	}
	...
	public function admin_replies_column_headers( $columns ) {
		...
		if ( !isset($_GET['post_status']) || 'reported' != $_GET['post_status'] || psf_get_reply_post_type() != get_current_screen()->post_type )
			return $columns;
	...
	public function admin_replies_column_data( $column, $reply_id ) {
		...
		switch ( $column ) {
			case 'psf_rc_user':
				$user_id = get_post_meta( $reply_id, '_psf_report_user_id', true );
				$user    = $this->get_username( $user_id );
				$link    = '<a class="psf-rc-user-col" href="' . admin_url( 'user-edit.php?user_id=' . intval($user_id) ) . '">' . esc_html( $user ) . '</a>';
				echo $link;
				break;
	...
	function get_username( $user_id ) {
		...
		if ( 0 != $user_id ) {
			...
			$username = __('Gast', 'psforum-report-content');
		}

		return $username;
	}

} // end class psf_ReportContent
